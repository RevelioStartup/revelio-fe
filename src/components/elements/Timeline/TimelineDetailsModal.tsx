...
const TimelineDetailsModal: React.FC<TimelineDetailsModalProps> = ({ timelineId, onClose, showModal, clickedEvent }) => {
  ...
  const [newStartDate, setNewStartDate] = React.useState<Dayjs | null>(dayjs(clickedEvent?.start?.toLocaleString()))
  const [newEndDate, setNewEndDate] = React.useState<Dayjs | null>(dayjs(clickedEvent?.end?.toLocaleString()))

  const [editDetailTimeline, { isLoading: modifyDetailTimelineLoading, isError, error }] =
    ...
    >
      {!isEditing ? (
        <Box sx={style}>
          ...
          <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 8, width: "fit-content" }}>
            <Button variant="outlined" color="error" onClick={handleDelete} disabled={isLoading}>
              Delete Timeline
            </Button>
            <Button variant="outlined" color = "primary" className="mt-4" onClick={() => setIsEditing(true)}>
                Edit Event
              </Button>
                  ...
                  onClick={async () => {
                    try {
                      const cleanedStartDatetime = newStartDate
                        ?.format('YYYY-MM-DD HH:mmZ') as string
