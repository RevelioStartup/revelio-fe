...
import { useGetTimelinesByEventQuery, useModifyDetailTimelineMutation } from '@/redux/api/timelineApi'
...
const DemoApp: React.FC<DemoAppProps> = ({ eventId }) => {
  ...
  const [ editDetailTimeline, { isLoading: modifyDetailTimelineLoading }] = useModifyDetailTimelineMutation()
            ...
            <Box sx = {style}> 
              ...
              <Box sx = {{ mt: 2}}>
                <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale='id'>       
                  <DatePicker label="Start Date" value={newStartDate}
              ...
              <Box sx={{ display: "flex", gap: 8, mt: 2 }}>
                <LoadingButton disabled = {modifyDetailTimelineLoading} onClick={() => setIsEditing(false)}>
                  Cancel
                </LoadingButton>
                <LoadingButton onClick={async () => {
                  const cleanedStartDatetime = newStartDate?.toISOString().split('T')[0] as string
                  const cleanedEndDatetime = newEndDate?.toISOString().split('T')[0] as string
