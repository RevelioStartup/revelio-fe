    ...
    return (
        ...
        <Box className="items-center justify-center flex my-4">
            ...
            <Box className="bg-teal-600 text-white p-6 rounded-3xl relative min-w-64 w-1/3">
                <span className="absolute top-0 right-0 bg-teal-100 rounded-2xl text-xs m-4 p-2 text-teal-600 font-bold">RECOMMENDED</span>
                <Box className="mx-4 my-8 space-y-3">
                <h2 className="text-3xl font-bold">Premium</h2>
                <p className='text-sm font-bold text-teal-100'>Limitless possibilities</p>
                {premium_package && <PackageCard package_detail={premium_package} />}
                </Box>
            </Box>
            </Box>
                ...
                <TableContainer component={Box} style={{ width: '75%' }}>
                    <Table>
                        <TableBody>
                            <TableRow>
                                <TableCell></TableCell>
                                <TableCell align="center"><h3 className='font-bold'>Free</h3></TableCell>
                                <TableCell align="center"><h3 className='font-bold'>Premium</h3></TableCell>
                            </TableRow>
                            {features.map((feature, index) => (
                                <TableRow key={feature} style={{ backgroundColor: index % 2 === 0 ? 'rgba(0,0,0,0.05)' : 'transparent' }}>
                                    <TableCell><p className='font-bold'>{capitalizeWords(feature)}</p></TableCell>
                                    <TableCell align="center">{free_package[feature] ? <i className="i-ph-check-circle-fill text-teal-600 size-6" /> : <i className="i-ph-x-circle-fill text-rose-500 size-6" />}</TableCell>
                                    <TableCell align="center">{premium_package[feature] ? <i className="i-ph-check-circle-fill text-teal-600 size-6" /> : <i className="i-ph-x-circle-fill text-rose-500 size-6" />}</TableCell>
                                </TableRow>
                            ))}
                            <TableRow>
                                <TableCell></TableCell>
                                ...
                                <TableCell align="center">
                                    ...
                                    (<button className="bg-white text-teal-600 border border-teal-600 rounded-2xl py-4 px-6 text-sm">Choose Plan <br/> ({formatRupiah(premium_package.price)})</button>) :
                                    <p className='text-teal-600'>Subscribed until {formatDate(latest_subscription.end_date)}</p>}</TableCell>
                            </TableRow>
