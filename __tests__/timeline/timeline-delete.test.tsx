...
describe('TimelineDetailsModal', () => {
  ...
  it('deletes the timeline and closes the modal on successful delete', async () => {
    ...
    global.confirm = jest.fn().mockReturnValue(true); // Mock confirmation to always return true

    fireEvent.click(getByText('Delete Timeline')); // Assuming the delete button has this text

    await waitFor(() => {
      expect(mockOnClose).toHaveBeenCalled(); // Check if onClose was called after successful delete
    });
  });
  ...
  it('does not delete the timeline if user cancels', async () => {
    ...
    global.confirm = jest.fn().mockReturnValue(false); // Mock confirmation to always return false

    fireEvent.click(getByText('Delete Timeline'));
    await waitFor(() => {
      expect(mockOnClose).not.toHaveBeenCalled(); // Ensure onClose has not been called
    });
  });
});
